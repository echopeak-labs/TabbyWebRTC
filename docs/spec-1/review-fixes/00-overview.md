...
Remediation specs for gaps, security issues, and flawed logic found when reviewing `docs/spec-1` against the implementation in `components/frontend`, `components/infra`, and `components/desktop-agent`.

Source review: 2026-07-10. Finding IDs are stable across this folder and must be referenced in PRs and `tasks.md`.
